...
This Shiny App is part of the supplementary materials of paper *Excess mortality in Europe during the 2020 COVID-19 pandemic: a multi-country study at the sub-national level* by *G. Konstantinoudis et al.* (2021). This paper estimates the excess mortality caused by the COVID-19 pandemic during 2020 in five European countries: England, Greece, Italy, Spain and Switzerland. The paper is currently available as a [pre-print](). 
...
'Inf' values may appear for some NUTS3 areas (or weeks in the temporal plots) for some combinations of age and gender when the expected number of deaths is zero. These values should be interpreted as that there is not enough information as to provide reliable estimates of the observed number of deaths.

# How to use the App?

Esentially, this App will allow you to navigate through the different excess mortality estimates discussed in the paper according to country, administrative level, age group and gender. The user can select these variables from the different drop-down menus on the left and the results can be accessed by clicking on the differnt tabs in the top-left part of the App. The different tabs are described below.
